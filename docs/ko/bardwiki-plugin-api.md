...
RisuVault의 Plugin API v3는 현재 채팅의 BardWiki 컨텍스트와 문서를 읽고 수정할 수 있는 `Risuai.bardWiki` API를 제공합니다. 기존 장기기억 필드를 읽는 플러그인은 코드를 바꾸지 않아도 BardWiki 컨텍스트를 받을 수 있습니다.

## 기존 장기기억 플러그인 호환

플러그인이 `Risuai.getCharacter()`, `Risuai.getChar()`, 인덱스 기반 캐릭터 또는 채팅 getter, 권한이 있는 데이터베이스 getter로 채팅을 읽으면 RisuVault가 다음 필드에 같은 가상 컨텍스트를 제공합니다.
