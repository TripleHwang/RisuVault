...
Có ba cách để di chuyển dữ liệu từ cài đặt RisuAI hiện có (RisuAI Web, RisuAI Cục bộ) sang RisuVault. Chọn dựa trên môi trường nguồn và kích thước dữ liệu của bạn.
...
Xuất tệp sao lưu `.bin` từ RisuAI hiện có, sau đó nhập vào RisuVault. Hoạt động bất kể môi trường nguồn (web / Tauri / Capacitor / cục bộ).

1. **Trong RisuAI hiện có**: Cài đặt > Sao lưu > "Lưu bản sao lưu cục bộ" để xuất tệp `.bin`.
2. **Trong RisuVault**: Cài đặt > Di chuyển dữ liệu > "Nhập bản sao lưu cục bộ Risu gốc" để nhập tệp `.bin`.
...
2. Trong RisuVault, vào Cài đặt > Di chuyển dữ liệu > accordion "Nhập thư mục save từ NodeOnly Risu".
...
1. Dừng máy chủ RisuVault.
2. Ghi đè thư mục `save` của RisuVault bằng thư mục `save` của RisuAI hiện có.
3. Khởi động lại máy chủ RisuVault — quá trình di chuyển tự động bắt đầu.
