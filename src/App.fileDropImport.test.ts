...
// The exported `readModule` is a saver-scope wrapper; the archive walk and
// its per-asset progress alerts live in `readModuleScoped`, which ends where
// the next top-level function begins.
const readModuleSource = moduleSource.slice(
    moduleSource.indexOf('async function readModuleScoped'),
    moduleSource.indexOf('export async function exportModuleLegacy')
)
