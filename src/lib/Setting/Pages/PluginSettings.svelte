<script lang="ts">
    ...
    async function removePlugins(pluginNames: readonly string[]) {
        ...
        // Permission grants are keyed by plugin name and outlive the plugin
        // unless they are cleared here. A later plugin installed under the same
        // name would inherit whatever this one was granted -- full database
        // access included -- without ever showing the user a consent dialog.
        for (const pluginName of pluginNamesToDelete) await resetPluginPermission(pluginName)
