...
function terms(value: string, minimumLength: number): string[] {
    // `?? ([] as string[])` rather than `?? []`: a bare `[]` is `never[]`, and
    // whether `RegExpMatchArray | never[]` narrows to `string[]` at the
    // `.filter` depends on the order the checker visits this program. Clean on
    // an upstream checkout, `term` is `never` in the merged one.
    return [...new Set((value.toLocaleLowerCase().match(termPattern) ?? ([] as string[])).filter((term) => term.length >= minimumLength))]
}
