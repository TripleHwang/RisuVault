/**
 * RisuVault NodeOnly — Korean help texts (`language.help`).
 *
...
export const helpKo = {
        ...
        "antiClaudeOverload": "Claude 과부하가 발생하면, RisuVault는 동일한 프롬프트로 계속 진행하여 과부하 발생 가능성을 줄이려고 시도합니다. 스트리밍 응답에서만 작동합니다. 공식 API 엔드포인트가 아닌 경우 작동하지 않을 수 있습니다.",
        ...
        "defaultVariables": "여기에서는 기본 변수를 정의할 수 있습니다. `<변수 이름>=<변수 값>` 형식으로 작성하고 개행으로 구분합니다. 예를 들어, `name=RisuVault`는 트리거 스크립트 및 변수 CBS와 함께 `{{getvar::A}}`, `{{setvar::A::B}}` 또는 `{{? $A + 1}}`과 같이 사용할 수 있습니다. 프롬프트 템플릿의 기본 변수와 캐릭터의 기본 변수가 동일한 이름을 가진 경우 캐릭터의 기본 변수가 사용됩니다.",
        ...
        "nodeOnlyStandardChatWidth": "RisuVault Standard 테마에서 메시지 카드의 최대 가로폭입니다.",
        ...
        "ollamaURL": "로컬 또는 원격 Ollama 서버의 URL (예: `http://localhost:11434`). RisuVault의 \"로컬 네트워크 모드\"와 함께 쓰면 사설망 LLM도 안정적으로 호출됩니다.",
        ...
        "memType": "장기 메모리 방식 선택.\n\n- **None**: 사용 안 함 (max context 한도까지 채팅 그대로 사용)\n- **HypaV3**: 오래된 채팅을 자동으로 요약·검색해 컨텍스트에 주입하는 RisuVault의 장기 메모리. 비용/지연이 약간 늘지만 긴 채팅에서 일관성이 크게 향상됩니다.",
