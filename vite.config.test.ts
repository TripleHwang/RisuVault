...
describe('development workflow', () => {
    ...
    test('uses port 6001 across supported server launch paths', () => {
        ...
        expect(server).toContain('const DEFAULT_PORT = 6001')
        expect(server).toContain('process.env.PORT || DEFAULT_PORT')
        expect(installScript).toContain('PORT="${PORT:-6001}"')
        expect(portableLauncher).toContain('wcscpy(port, L"6001")')
        expect(termuxBuild).toContain('http://localhost:6001')
        expect(dockerfile).toContain('EXPOSE 6001')
        expect(compose).toContain('- 6001:6001')
    })
