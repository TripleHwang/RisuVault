...
test.skipIf(process.platform !== 'win32')('detached Windows helper waits for shutdown, installs and starts the replacement server', async () => {
    ...
    try {
        ...
        write(root, 'RisuVault.exe', 'existing launcher')
        ...
        write(source, 'RisuVault.exe', 'replacement launcher must not overwrite running launcher')
        ...
        expect(fs.readFileSync(path.join(root, 'RisuVault.exe'), 'utf8')).toBe('existing launcher')
